...
- "femaleAppearance": MUST start with "[NAME], a [AGE]-year-old". Use the characterName. Age: TRULY RANDOM between 18-65 (~40% ages 18-30, ~30% ages 31-45, ~30% ages 46-65). Do NOT default to older women every time. Vary body type (slim, athletic, curvy, petite, tall, chubby, plump). Vary hair (blonde, black, red, grey, silver, pixie, braids, curly). Describe her EVERYDAY JOB. 4-6 specific physical details. She should look REAL, not like a model.
...
- At the very end, add: SEO_DESC: [SEO meta description, 150-160 chars]`;
  ...
  if (process.env.VENICE_API_KEY) {
    ...
    if (faceDescription) {
      try {
        const pAgeMatch = generatedAppearance?.match(/(\d+)-year-old/);
